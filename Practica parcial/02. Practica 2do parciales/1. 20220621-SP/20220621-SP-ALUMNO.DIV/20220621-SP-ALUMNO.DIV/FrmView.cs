...
namespace _20220621_SP_ALUMNO.DIV
{
    ...
    public partial class FrmView : Form
    {
        ...
        {
            //Alumnos: Instanciar Ahorcado y agregar manejadores
        ...
        private void MostrarConteo(int segundo)
        {
            ...
            {
                //this.BeginInvoke(() => this.MostrarConteo(segundo));
                this.BeginInvoke(new Action<int>(this.MostrarConteo), new object[] { segundo });
            }
            else
            {
        ...
        private void MostrarPalabra(string palabra)
        {
            if (this.InvokeRequired)
            {
                this.BeginInvoke(new Action<string>(this.MostrarPalabra), new object[] { palabra });
            }
            else
            {
        ...
        private void FrmView_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                FileManager.Guardar(this.ahorcado, "/ahorcado.JSON");
            }
            catch (Exception ex)
            {
